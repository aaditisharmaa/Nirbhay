  ...
  const handleSearchRoutes = async (e, modeOverride) => {
    ...
    try {
      ...
      if (startQuery && startQuery !== 'Current Location') {
        ...
        if (!Array.isArray(startData) || !startData.length) throw new Error('Start location not found.');
      ...
      if (destQuery && destQuery !== 'Hauz Khas Village') {
        ...
        if (!Array.isArray(geoData) || !geoData.length) throw new Error('Destination not found.');
